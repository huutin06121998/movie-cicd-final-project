...
You have been recruited as the DevOps specialist for a development team managing a web application that serves as a catalog for Movie Picture movies. The team urgently needs to automate their development workflows to accelerate their release cycle. They are seeking to implement GitHub Actions for automating the testing, building, and deployment of their applications to an existing Kubernetes cluster.

The team's project consists of two applications:

1. A frontend UI written in TypeScript, using the React framework.
2. A backend API written in Python, using the Flask framework.

In the `starter` folder, you will find two directories: frontend and backend, each containing the source code for their respective applications. Your task is to use the team's existing documentation to create CI/CD pipelines that fulfill the team's requirements.
